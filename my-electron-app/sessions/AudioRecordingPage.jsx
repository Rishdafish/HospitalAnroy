...
const AudioRecordingPage = ({ patientId, sessionType, sessionData }) => {
  ...
  useEffect(() => {
    ...
    console.log('🔄 [AudioRecordingPage] Loading session, data type:', typeof sessionData);
    
    if (sessionData) {
      console.log('🔄 [AudioRecordingPage] Session data available:', 
        typeof sessionData === 'string' ? sessionData : 'object');

      // Check if this is a session ID being passed instead of full session data
      if (typeof sessionData === 'string') {
        console.log('🔄 [AudioRecordingPage] Looking up session by ID:', sessionData);
        const sessionById = SessionService.getSession(sessionData);
        console.log('🔄 [AudioRecordingPage] Session retrieved:', !!sessionById);
        
        if (sessionById) {
          sessionData = sessionById;
          setSessionObj(sessionById);
          console.log('🔄 [AudioRecordingPage] Session notes length:', 
            sessionById.notes ? sessionById.notes.length : 0);
        } else {
          console.error('🔄 [AudioRecordingPage] Session not found with ID:', sessionData);
        }
      } else {
        console.log('🔄 [AudioRecordingPage] Using session object directly');
      ...
      if (hasTemplateId || hasTemplateInSessionType) {
        ...
        console.log('🔄 [AudioRecordingPage] Detected custom template in field:', 
          hasTemplateId ? 'templateId' : 'sessionType');
        console.log('🔄 [AudioRecordingPage] Template ID value:', effectiveTemplateId);
        ...
        console.log('🔄 [AudioRecordingPage] Raw template ID:', templateId);
        
        // We'll use a multi-phase approach to get the template content
        console.log('🔄 [AudioRecordingPage] Phase 1: Check TemplateService cache');
        let templateFound = false;
        let templateData = TemplateService.getTemplateSync(templateId);
        
        if (templateData && templateData.format) {
          console.log('🔄 [AudioRecordingPage] Phase 1 SUCCESS - Found template format in cache, length:', 
            templateData.format.length);
          console.log('🔄 [AudioRecordingPage] Format preview:', templateData.format.substring(0, 100) + '...');
          templateFound = true;
          
          // Apply this format to our session notes immediately
          if (!sessionData.notes || sessionData.notes.trim() === '') {
            console.log('🔄 [AudioRecordingPage] Updating session with template content');
          ...
          console.log('🔄 [AudioRecordingPage] Phase 1 FAILED - Template not in TemplateService cache');
        }
        
        if (!templateFound) {
          console.log('🔄 [AudioRecordingPage] Phase 2: Direct database.db access');
          try {
            const dbStr = localStorage.getItem('database.db');
            if (dbStr) {
              console.log('🔄 [AudioRecordingPage] Found database.db in localStorage');
              const dbData = JSON.parse(dbStr);
              
              if (dbData && dbData.templates && Array.isArray(dbData.templates)) {
                console.log('🔄 [AudioRecordingPage] database.db has', dbData.templates.length, 'templates');
                console.log('🔄 [AudioRecordingPage] Template IDs in database:', dbData.templates.map(t => t.id));
                
                const template = dbData.templates.find(t => t.id === templateId);
                
                if (template && template.format) {
                  console.log('🔄 [AudioRecordingPage] Phase 2 SUCCESS - Found template in database.db');
                  templateFound = true;
                  console.log('🔄 [AudioRecordingPage] Format preview:', template.format.substring(0, 100) + '...');
                  
                  // Apply this format to our session notes immediately
                  if (!sessionData.notes || sessionData.notes.trim() === '') {
                    console.log('🔄 [AudioRecordingPage] Updating session with template content from database');
                  ...
                  console.log('🔄 [AudioRecordingPage] Phase 2 FAILED - Template not found in database.db');
                }
              }
            }
          } catch (e) {
            console.error('🔄 [AudioRecordingPage] Phase 2 ERROR:', e);
          }
        }
        
        if (!templateFound) {
          console.log('🔄 [AudioRecordingPage] Phase 3: Try async loading');
          TemplateService.getTemplate(templateId).then(template => {
            if (template && template.format) {
              console.log('🔄 [AudioRecordingPage] Phase 3 SUCCESS - Template loaded asynchronously');
              console.log('🔄 [AudioRecordingPage] Format length:', template.format.length);
              console.log('🔄 [AudioRecordingPage] Format preview:', template.format.substring(0, 100) + '...');
              
              // Apply this format to our session notes
              if (!sessionData.notes || sessionData.notes.trim() === '') {
                console.log('🔄 [AudioRecordingPage] Updating session with template content from async load');
                sessionData.notes = template.format;
                setSessionNotes({
                  data: template.format,
                  assessment: '',
              ...
              console.log('🔄 [AudioRecordingPage] Phase 3 FAILED - Async load returned empty template');
            }
          }).catch(err => {
            console.error('🔄 [AudioRecordingPage] Phase 3 ERROR:', err);
          });
        }
      }
      ...
      console.log('🔄 [AudioRecordingPage] Processing session notes, available:', !!sessionData.notes);
      if (sessionData.notes) {
        console.log('🔄 [AudioRecordingPage] Notes length:', sessionData.notes.length);
      }
      
      let cleanNotes = sessionData.notes;
      if (metadataMatch) {
        console.log('🔄 [AudioRecordingPage] Found metadata in notes, cleaning');
      ...
      if ((!cleanNotes || cleanNotes.trim() === '') && 
        ...
        console.log('🔄 [AudioRecordingPage] EMERGENCY: Session still has no notes, trying direct template access');
        ...
        console.log('🔄 [AudioRecordingPage] EMERGENCY: Using template source:', emergencyHasTemplateId ? 'templateId' : 'sessionType');
        
        try {
          const dbStr = localStorage.getItem('database.db');
          if (dbStr) {
            const dbData = JSON.parse(dbStr);
            console.log('🔄 [AudioRecordingPage] EMERGENCY: database.db parsed');
            
            if (dbData && dbData.templates) {
              console.log('🔄 [AudioRecordingPage] EMERGENCY: Found templates:', dbData.templates.length);
              console.log('🔄 [AudioRecordingPage] EMERGENCY: Template IDs:', dbData.templates.map(t => t.id).join(', '));
              ...
              if (!template) {
                ...
                console.log('🔄 [AudioRecordingPage] EMERGENCY: Trying with prefixed ID:', prefixedId);
              }
              
              // Last attempt - try matching ID substring
              if (!template) {
                console.log('🔄 [AudioRecordingPage] EMERGENCY: Trying substring match as last resort');
                template = dbData.templates.find(t => t.id.includes(templateId));
              }

              if (template && template.format) {
                console.log('🔄 [AudioRecordingPage] EMERGENCY: Found template!', template.id);
                console.log('🔄 [AudioRecordingPage] EMERGENCY: Content:', template.format.substring(0, 50));
                cleanNotes = template.format;
                
                // Also save it back to the session
                SessionService.updateSession(sessionData.id, { notes: template.format });
              } else {
                console.log('🔄 [AudioRecordingPage] EMERGENCY: Template not found after all attempts');
          ...
          console.error('🔄 [AudioRecordingPage] EMERGENCY template access error:', e);
        }
      }
                  ...
                  <StructuredTemplateForm 
                    ...
                    onChange={(formValues) => {
                      ...
                      if (contentLength % 10 === 0) {
                        console.log("🔄 [AudioRecordingPage] Form updated, content length:", contentLength);
                      }
                    ...
                    <div>
                      ...
                      {sessionNotes.data === "chickens like chickens" && (
                        <div className="mt-2 p-2 bg-green-100 border border-green-300 text-green-800 rounded">
                          ✓ Custom note content successfully applied!
                        </div>
                      )}
                    </div>
                  </div>
